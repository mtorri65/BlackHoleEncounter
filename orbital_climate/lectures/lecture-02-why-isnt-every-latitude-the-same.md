...
**Next lecture:** we have insolation everywhere on the planet, and from Lecture 1 we know how to turn absorbed energy into a temperature. Time to connect them. But there's a complication we've ignored: the tropics receive far more energy than they radiate, and the poles radiate more than they receive — yet neither is boiling or endlessly cooling. Something is moving heat polewards, and we'll need to represent it. We'll also meet ice, which reflects sunlight, which causes cooling, which makes more ice — the first feedback in the course, and the one that can flip a planet into a snowball.
